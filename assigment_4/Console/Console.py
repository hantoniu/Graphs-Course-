...
class Console:
    ...
    def printCommands():
        ...
        print("Return the minimum-cost distance between vertices START END: distance START END")
        print("Topological sorting: topological")
        print("Prints the Minimum Spanning Tree: mst")
        print("Exit: exit\n")
    ...
    def __ui_Dijkstra(self, commands):
        start = 0
        end = 0
        try:
            ...
            return
        try:
            [cost, path] = (self.__controller.Dijkstra(start, end))
            if cost == -1:
                print("There is no path between these nodes!")
                return
            print (cost)
            result_string = ""
            for vertex in path:
    ...
    def __ui_topologicalSorting(self):
        lst = self.__controller.topologicalSorting()
        ...
        result_str = ""
        for vertex in lst:
            result_str += str(vertex) + " "
        print(result_str)
    
    def __ui_MST(self):
        lst  = self.__controller.kruskalAlgorithm()
        
        if lst[0] == -1:
            print("A minimum spanning tree could not be created!")
            return
    ...
    def run(self):
        while True:
            ...
            elif commands[0] == "distance" and len(commands) == 3:
                self.__ui_Dijkstra(commands)
            elif commands[0] == "topological" and len(commands) == 1:
                self.__ui_topologicalSorting()
            elif commands[0] == "mst" and len(commands) == 1:
                self.__ui_MST()
